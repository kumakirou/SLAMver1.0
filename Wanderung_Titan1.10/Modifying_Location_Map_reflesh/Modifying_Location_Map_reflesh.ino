...
関数‥map_reforming,MOD_LOC,SD_read_map,max,qmc5883,ultrasonic(),importing,exporting,possibility_theta,r_theta_table
*/
...
/*sub関数(自作)*/
void map_reforming(short int loc_x,short int loc_y,char D[div_theta+1]){
    bloodhound(D);
...
void MOD_LOC(short int loc_x,short int loc_y,int loc[2],char D[div_theta+1]){
    short int theta_C;
